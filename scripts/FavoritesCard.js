...
cardItems = [ new FavoritesCard("Unleashia kushon", 25, "https://korealab.by/image/cache/catalog/unleashia/stojkij-tonalnyj-kushon-unleashia-healthy-green-cushion1-800x800.jpg"), 
...
if (window.location.pathname.includes("favorites.html")) {
  ...
  document.addEventListener("click", function(e) {
    if (e.target.classList.contains("add-to-cart")) {
      ...
      const price = itemElement.querySelector("p:nth-child(2)").textContent.replace("Price: ", "").replace("$", "");
      const img = itemElement.querySelector("img").src;

      const cartItems = JSON.parse(localStorage.getItem("cartItems")) || [];
      cartItems.push({ name: name, price: price, imgSrc: img });
